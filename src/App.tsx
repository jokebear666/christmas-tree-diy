...
const PhotoOrnaments = ({ state, photoUrls, count }: { state: 'CHAOS' | 'FORMED', photoUrls: string[], count: number }) => {
...
const Experience = ({ sceneState, rotationSpeed, photoUrls, counts }: { sceneState: 'CHAOS' | 'FORMED', rotationSpeed: number, photoUrls: string[], counts: { foliage: number; ornaments: number; elements: number; lights: number } }) => {
    ...
    <>
      ...
      <group position={[0, -6, 0]}>
        ...
        <Suspense fallback={null}>
           <PhotoOrnaments state={sceneState} photoUrls={photoUrls} count={counts.ornaments} />
    ...
    <div style={{ width: '100vw', height: '100vh', backgroundColor: '#000', position: 'relative', overflow: 'hidden' }}>
      <div style={{ width: '100%', height: '100%', position: 'absolute', top: 0, left: 0, zIndex: 1 }}>
        <Canvas dpr={[1, 2]} gl={{ toneMapping: THREE.ReinhardToneMapping }} shadows>
            <Experience sceneState={sceneState} rotationSpeed={rotationSpeed} photoUrls={photoUrls} counts={counts} />
        </Canvas>
      </div>
      ...
      <div style={{ position: 'absolute', top: '20px', right: '20px', zIndex: 12, display: 'flex', gap: '8px' }}>
      ...
      {/* 参数面板 */}
      {showSettings && (
        <div style={{ position: 'absolute', top: '60px', right: '20px', zIndex: 13, width: '280px', background: 'rgba(0,0,0,0.7)', border: '1px solid rgba(255,215,0,0.5)', color: '#FFD700', padding: '12px', borderRadius: '8px', backdropFilter: 'blur(6px)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 90px', rowGap: '8px', columnGap: '8px' }}>
            <label>树叶粒子数量（越大越密，性能压力增）</label>
            <input type="number" min={1000} max={80000} step={500} value={counts.foliage} onChange={(e) => setCounts(c => ({ ...c, foliage: Math.max(1000, Math.min(80000, Number(e.target.value) || 0)) }))} style={{ width: '100%' }} />

            <label>拍立得照片数量（可超过照片数，循环使用）</label>
            <input type="number" min={10} max={1000} step={10} value={counts.ornaments} onChange={(e) => setCounts(c => ({ ...c, ornaments: Math.max(1, Math.min(2000, Number(e.target.value) || 0)) }))} style={{ width: '100%' }} />

            <label>圣诞元素数量（礼物盒、球、拐杖糖）</label>
            <input type="number" min={10} max={1000} step={10} value={counts.elements} onChange={(e) => setCounts(c => ({ ...c, elements: Math.max(0, Math.min(2000, Number(e.target.value) || 0)) }))} style={{ width: '100%' }} />

            <label>彩灯数量（闪烁灯泡数）</label>
            <input type="number" min={10} max={1000} step={10} value={counts.lights} onChange={(e) => setCounts(c => ({ ...c, lights: Math.max(0, Math.min(2000, Number(e.target.value) || 0)) }))} style={{ width: '100%' }} />
          </div>
          <p style={{ marginTop: '10px', color: '#ccc', fontSize: '11px' }}>提示：数值越大视觉越华丽，但在手机端可能卡顿。建议逐步调试找到合适的平衡。</p>
        </div>
      )}

      {/* 说明面板 */}
      {showHelp && (
        <div style={{ position: 'absolute', top: '60px', right: '20px', zIndex: 13, width: '360px', background: 'rgba(0,0,0,0.7)', border: '1px solid rgba(255,215,0,0.5)', color: '#FFD700', padding: '12px', borderRadius: '8px', backdropFilter: 'blur(6px)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
          ...
          <div style={{ color: '#eee', fontSize: '12px', lineHeight: 1.6 }}>
            <p>🖐 张开手掌（Open Palm）→ Disperse（散开）：圣诞树炸裂成漫天飞舞的粒子和照片。</p>
            <p>✊ 握紧拳头（Closed Fist）→ Assemble（聚合）：所有元素瞬间聚合成一棵完美的圣诞树。</p>
            <p>👋 手掌左右移动 → 旋转视角：手向左移，树向左转；手向右移，树向右转。</p>
            <p>👋 手掌上下移动 → 俯仰视角：手向上移，视角抬高；手向下移，视角降低。</p>
            ...
            <p>🔒 权限提示：需要摄像头权限才能使用手势控制；在线部署需在 HTTPS 环境（如 GitHub Pages）。</p>
          </div>
        </div>
      )}
    </div>
